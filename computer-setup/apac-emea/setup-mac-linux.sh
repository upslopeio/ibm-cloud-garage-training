...
function has_missing_tools {
  TOOLS=(brew docker git ibmcloud gh icc igc jq kubectl kustomize node npm oc tkn yq);
  INSTALLED=0;
  MISSING='';
  ...
  if [[ -z $MISSING ]]; then
    printf "Found all the following tools: %s\n" "${TOOLS[*]}";
    return 1;
  fi
...
function configure_git {
  ...
  GIT_USER_NAME=$(git config --global user.email);
  set -e
  ...
  do
    ...
    read -rp "Please Enter your email address for git: " GIT_EMAIL
  done
  ...
  do
    ...
    read -rp "Please Enter your user name for git: " GIT_USER_NAME
  done
...
function has_missing_icc_account {
  if [ -f "${ICC_HOME}/ibmcloud-account.yaml" ]; then
    API_KEY=$(yq eval ".accounts.upslope-garage" "${ICC_HOME}/ibmcloud-account.yaml" )
  fi
  ...
  if [ -z "${API_KEY}" ]; then
    ...
    do
      ...
      read -rp "Please enter your IBM cloud API key: " API_KEY
    done

    touch "${ICC_HOME}/ibmcloud-account.yaml"
    yq e -i ".accounts.upslope-garage=\"${API_KEY}\"" "${ICC_HOME}/ibmcloud-account.yaml"
...
if has_missing_icc_account; then
  add_icc_account
fi
